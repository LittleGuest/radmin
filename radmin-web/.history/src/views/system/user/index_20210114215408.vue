...
let userRoles = []
let userJobs = []
const defaultForm = { id: null, username: null, nick_name: null, gender: '男', email: null, enabled: 0, roles: [], jobs: [], dept: { id: null }, phone: null }
export default {
  name: 'User',
  ...
  methods: {
    ...
    getSupDepts(dept_id) {
      getDeptSuperior(dept_id).then(res => {
        this.buildDepts(res)
        this.depts = res
      })
    },
    buildDepts(depts) {
      depts.forEach(data => {
        ...
        if (data.has_child && !data.children) {
          data.children = null
        }
      })
    },
